// Contas a Receber — lista, calendário, fluxo e ações de recebimento.
(function () {
  const CATEGORIES = {
    vendas: "Vendas",
    servicos: "Serviços",
  ...
  function statusBadge(status) {
    ...
    return `<span class="ar-badge ${map[status] || "badge-muted"}">${STATUS_LABELS[status] || status}</span>`;
  }

  async function loadCustomers() {
    try {
      const res = await BuildFlow.apiFetch("/customers?limit=all");
      customers = res.data || [];
    } catch (_) {
      customers = [];
    }
    if (els.fieldCustomer) {
      els.fieldCustomer.innerHTML =
        `<option value="">Sem cliente vinculado</option>` +
        customers
          .filter((c) => c.active !== false)
          .map((c) => `<option value="${BuildFlow.escapeHtml(normalizeId(c._id))}">${BuildFlow.escapeHtml(c.name)}</option>`)
          .join("");
    }
  }
  ...
  async function loadData() {
    ...
    const [listRes, summaryRes] = await Promise.all([
      BuildFlow.apiFetch(`/accounts-receivable?${params.toString()}`),
      BuildFlow.apiFetch(`/accounts-receivable?${summaryParams.toString()}`),
    ]);
  ...
  function renderKpis() {
    ...
    els.kpiPaid.textContent = BuildFlow.formatCurrency(summary.receivedThisMonth);
    ...
    if (summary.countOverdue > 0) {
      ...
      els.alertBanner.innerHTML = `
        ...
        <div>
          <strong>${summary.countOverdue} recebimento(s) vencido(s)</strong>
          <span>Total em atraso: ${BuildFlow.formatCurrency(summary.totalOverdue)}</span>
        </div>
        <button type="button" class="btn btn-sm" id="filterOverdueBtn">Ver vencidos</button>`;
  ...
  function renderTable() {
    ...
    if (!bills.length) {
      els.tableBody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--text-muted)">Nenhum recebimento encontrado. Cadastre seu primeiro.</td></tr>`;
      return;
    }
    ...
    els.tableBody.innerHTML = bills
      .map((bill) => {
        ...
        return `<tr class="ar-row ar-row--${bill.status}" data-bill-id="${id}" style="cursor:pointer;">
          ...
          <td>${BuildFlow.escapeHtml(bill.customerName || "—")}</td>
          ...
          <td class="ar-actions">
            ${bill.status !== "received" && bill.status !== "cancelled" ? `<button type="button" class="btn-icon" data-receive="${id}" title="Marcar como recebido"><i class="fa-solid fa-check"></i></button>` : ""}
  ...
  function renderCalendar() {
    ...
    let html = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"]
      .map((d) => `<div class="ar-cal-head">${d}</div>`)
      .join("");

    for (let i = 0; i < startWeekday; i++) html += `<div class="ar-cal-cell ar-cal-cell--empty"></div>`;
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      html += `<div class="ar-cal-cell${isToday ? " ar-cal-cell--today" : ""}${hasOverdue ? " ar-cal-cell--overdue" : ""}" data-day="${day}">
        <span class="ar-cal-day">${day}</span>
        ${dayBills.length ? `<span class="ar-cal-count">${dayBills.length}</span><span class="ar-cal-total">${BuildFlow.formatCurrency(total)}</span>` : ""}
      </div>`;
    }
  ...
  function renderCashFlow() {
    ...
    els.cashFlowChart.innerHTML = summary.cashFlow
      .map((day) => {
        ...
        const cls = day.total > 0 ? "ar-bar--active" : "";
        return `<div class="ar-bar-wrap" title="${label}: ${BuildFlow.formatCurrency(day.total)} (${day.count})">
          <div class="ar-bar ${cls}" style="height:${height}%"></div>
          <span>${label}</span>
        </div>`;
      })
      .join("");
  ...
  function showDayBills(day) {
    ...
    const rows = dayBills
      .map((b) => {
        return `<tr>
          ...
          <td style="padding:10px 8px;white-space:nowrap;">${BuildFlow.escapeHtml(b.customerName || "—")}</td>
    ...
    Swal.fire({
      ...
      html: `<div style="text-align:left;max-height:55vh;overflow-y:auto;"><table style="width:100%;border-collapse:collapse;font-size:0.85rem;table-layout:fixed;">
        <colgroup><col><col style="width:130px;"><col style="width:100px;"><col style="width:90px;"><col style="width:90px;"></colgroup>
        <thead><tr style="border-bottom:2px solid var(--border);text-align:left;">
          <th style="padding:10px 8px;">Descrição</th>
          <th style="padding:10px 8px;white-space:nowrap;">Cliente</th>
  ...
  function showInstallmentGroup(groupId) {
    BuildFlow.apiFetch(`/accounts-receivable?groupId=${encodeURIComponent(groupId)}`).then((groupBills) => {
      if (!groupBills || !groupBills.length) return;
      const rows = groupBills
        .map((b) => {
          const done = b.status === "received" || b.status === "cancelled";
          return `<tr style="border-bottom:1px solid var(--border);${done ? "opacity:0.6;" : ""}">
      ...
      Swal.fire({
        ...
        html: `<div style="text-align:left;"><div style="margin-bottom:12px;font-size:0.85rem;color:var(--text-muted);">${receivedCount} de ${groupBills.length} recebidas · Total: ${BuildFlow.formatCurrency(groupBills.reduce((s, b) => s + (Number(b.amount) || 0), 0))}</div><div style="max-height:50vh;overflow-y:auto;"><table style="width:100%;border-collapse:collapse;font-size:0.85rem;table-layout:fixed;">
          ...
          <thead><tr style="border-bottom:2px solid var(--border);">
            ...
            <th style="padding:10px 8px;text-align:right;white-space:nowrap;">Recebido em</th>
            <th style="padding:10px 8px;white-space:nowrap;">Status</th>
          </tr></thead>
          <tbody>${rows}</tbody>
        </table></div></div>`,
  ...
  async function showBillDetails(bill) {
    ...
    if (isGroup) {
      try {
        const res = await BuildFlow.apiFetch(`/accounts-receivable?groupId=${encodeURIComponent(bill.installmentGroupId)}`);
        if (res && res.length) groupData = res;
      } catch (_) {}
    }

    const receivedBills = groupData ? groupData.filter((b) => b.status === "received") : [];
    const pendingBills = groupData ? groupData.filter((b) => b.status !== "received" && b.status !== "cancelled") : [];

    let summaryHtml = "";
    if (groupData) {
      const totalAmount = groupData.reduce((s, b) => s + (Number(b.amount) || 0), 0);
      const receivedTotal = receivedBills.reduce((s, b) => s + (Number(b.amount) || 0), 0);
      ...
      summaryHtml = `
        <div style="display:flex;gap:16px;margin-bottom:16px;padding:12px 16px;background:var(--bg-hover);border-radius:10px;">
          <div style="flex:1;text-align:center;">
            <div style="font-size:0.7rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.04em;">Recebidas</div>
            <div style="font-size:1.1rem;font-weight:700;color:#10b981;">${receivedBills.length}/${groupData.length}</div>
            <div style="font-size:0.8rem;color:var(--text-secondary);">${BuildFlow.formatCurrency(receivedTotal)}</div>
          </div>
          <div style="flex:1;text-align:center;">
    ...
    const parcelas = isGroup ? `${bill.installmentNumber || "—"}/${bill.totalInstallments}` : "—";
      ...
      <div style="text-align:left;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <tbody>
            ...
            ${renderRow("Cliente", BuildFlow.escapeHtml(bill.customerName || "—"))}
            ...
            ${bill.paymentMethod ? renderRow("Forma de Recebimento", BuildFlow.escapeHtml(PAYMENT_METHODS[bill.paymentMethod] || bill.paymentMethod)) : ""}
            ${bill.receivedDate ? renderRow("Recebido em", formatDate(bill.receivedDate)) : ""}
          </tbody>
        </table>
  ...
  function openModal(bill = null) {
    ...
    els.modalTitle.textContent = bill ? "Editar Recebimento" : "Nova Conta a Receber";
    els.fieldDescription.value = bill?.description || "";
    if (bill?.customerId) {
      els.fieldCustomer.value = normalizeId(bill.customerId);
    } else {
      els.fieldCustomer.value = "";
    }
    els.fieldCategory.value = bill?.category || "vendas";
    const isInstallment = bill?.totalInstallments > 1;
    const installmentAmount = isInstallment && bill?.totalInstallments ? (Number(bill.amount) || 0) : (bill?.amount ?? "");
    els.fieldAmount.value = installmentAmount;
    els.fieldDueDate.value = bill?.dueDate ? new Date(bill.dueDate).toISOString().slice(0, 10) : "";
    els.fieldPaymentMethod.value = bill?.paymentMethod || "pix";
    ...
    if (els.installmentFields) els.installmentFields.style.display = "block";
    ...
    if (els.installmentCountFields) els.installmentCountFields.style.display = isInstallment ? "block" : "none";
  ...
  async function saveBill(e) {
    ...
    const payload = {
      ...
      customerId: els.fieldCustomer.value || null,
    ...
    try {
      if (editingId) {
        ...
        await BuildFlow.apiFetch("/accounts-receivable", {
          method: "PUT",
          body: JSON.stringify({ id: editingId, ...editPayload, amount: Number(els.fieldAmount.value) }),
        });
        Swal.fire({ icon: "success", title: "Recebimento atualizado!", timer: 1800, showConfirmButton: false });
      } else {
        await BuildFlow.apiFetch("/accounts-receivable", {
          method: "POST",
          body: JSON.stringify(payload),
        });
        const msg = totalInstallments > 1 ? `${totalInstallments} parcelas cadastradas!` : "Recebimento cadastrado!";
  ...
  async function markReceived(id) {
    const result = await Swal.fire({
      ...
      text: "A conta será marcada como recebida.",
      icon: "question",
    ...
    try {
      await BuildFlow.apiFetch("/accounts-receivable", {
        method: "PUT",
        body: JSON.stringify({ id, action: "receive", receivedDate: getLocalDateString() }),
      });
      Swal.fire({ icon: "success", title: "Recebimento registrado!", timer: 1600, showConfirmButton: false });
      await loadData();
    } catch (err) {
      Swal.fire({ icon: "error", title: "Erro", text: err.message });
    }
  ...
  function checkBrowserNotifications() {
    ...
    const key = "ar_notified_" + new Date().toISOString().slice(0, 10);
    ...
    const notify = () => {
      ...
      let body = "";
      if (overdue) body += `${overdue} recebimento(s) vencido(s). `;
      if (soon) body += `${soon} vencimento(s) nos próximos dias.`;
      new Notification("BuildFlow — Contas a Receber", { body, icon: "/favicon.ico" });
  ...
  function bindEvents() {
    ...
    document.querySelectorAll(".ar-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        document.querySelectorAll(".ar-panel").forEach((p) => {
    ...
    els.tableBody?.addEventListener("click", (e) => {
      ...
      if (receive) { markReceived(receive.dataset.receive); return; }
      if (edit) {
